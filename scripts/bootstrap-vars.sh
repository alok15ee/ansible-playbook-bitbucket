...
sed -i "s/^#\(apache2_vhosts_proxy_pass\):.*/#\1: \/   http:\/\/localhost:7990\//g" $TMP_VARS
sed -i "s/^#\(apache2_vhosts_proxy_pass_reverse\):.*/#\1: \/   http:\/\/localhost:7990\//g" $TMP_VARS
...
sed -i "s/^#\(apache2_vhosts_server_admin\):.*/\1: \"webmaster@{{ bitbucket_proxy_name }}\"/g" $TMP_VARS
sed -i "s/^#\(apache2_vhosts_server_alias\):.*/\1: []/g" $TMP_VARS
sed -i "s/^#\(apache2_vhosts_server_name\):.*/\1: \"{{ bitbucket_proxy_name }}\"/g" $TMP_VARS
...
sed -i "s/^#\(bitbucket_user\):.*/\1: bitbucket/g" $TMP_VARS
...
sed -i "s/^#\(mysql_db_name\):.*/#\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
sed -i "s/^#\(mysql_user_name\):.*/#\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
sed -i "s/^#\(mysql_user_password\):.*/#\1: \"{{ bitbucket_pass }}\"/g" $TMP_VARS
sed -i "s/^#\(postgresql_db_encoding\):.*/\1: UTF8/g" $TMP_VARS
sed -i "s/^#\(postgresql_db_name\):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
sed -i "s/^#\(postgresql_db_owner\):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
sed -i "s/^#\(postgresql_db_template\):.*/\1: template0/g" $TMP_VARS
sed -i "s/^#\(postgresql_user_name\):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
sed -i "s/^#\(postgresql_user_password\):.*/\1: \"{{ bitbucket_pass }}\"/g" $TMP_VARS
